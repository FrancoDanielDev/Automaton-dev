...
public class SpatialGrid : MonoBehaviour
{
    ...
    public Vector3 ObjectBounds(Vector3 objectPos)
    {
        float zBound = cellHeight;
        float xBound = cellWidth;
        if (objectPos.z > zBound) objectPos.z = -zBound;
        if (objectPos.z < -zBound) objectPos.z = zBound;
        if (objectPos.x > xBound) objectPos.x = -xBound;
        if (objectPos.x < -xBound) objectPos.x = xBound;
    ...
    {
        var lastPos = lastPositions[entity];
        buckets[lastPos.Item1, lastPos.Item2].Remove(entity);
        lastPositions.Remove(entity);
    }
